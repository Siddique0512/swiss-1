...
async function main() {
  const contract = await hre.ethers.deployContract("Swisstronik", ["Hello Swisstronik from Siddique"]);
...
async function main() {
  ...
  const messageToSet = "Hello Swisstronik from Siddique";
...
echo "Done! Swisstronik Contract is successfully deployed on hardhat :) -> By Siddique"